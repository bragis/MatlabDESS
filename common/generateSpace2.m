...
%
% TRHi: Repetition time of "hi" sequence (seconds)
% TEHi: Echo time of "hi" sequence (seconds)
% TgHi: Gradient duration of "hi" sequency (seconds)
% TRLo: Repetition time of "hi" sequence (seconds)
% TELo: Echo time of "hi" sequence (seconds)
% TgLo: Gradient duration of "hi" sequency (seconds)
...
function [T1T2DDspace] = generateSpace2(T1start,T1end,NT1points,T2start,...
    T2end,NT2points,DDstart,DDend,NDDpoints,TRHi,TEHi,TgHi,alpha1,G1,...
    TRLo,TELo,TgLo,alpha2,G2,Nstates,signalModel)
...
for (i=1:NT1points)
    ...
    for (j=1:NT2points)
        ...
        for (k=1:NDDpoints)
            ...
            if (signalModel == 0)
                [S1p,S1m] = computeEchoesEPG(T1_ijk,T2_ijk,TRHi,TEHi,alpha1,G1,TgHi,DD_ijk,Nstates);
                [S2p,S2m] = computeEchoesEPG(T1_ijk,T2_ijk,TRLo,TELo,alpha2,G2,TgLo,DD_ijk,Nstates);
            else
                [S1p,S1m] = computeEchoesWuBuxton(alpha1,TRHi,G1,TgHi,(TRHi-TgHi)/2,TEHi,2*TRHi-TEHi,T1_ijk,T2_ijk,DD_ijk);
                [S2p,S2m] = computeEchoesWuBuxton(alpha2,TRLo,G2,TgLo,(TRLo-TgLo)/2,TELo,2*TRLo-TELo,T1_ijk,T2_ijk,DD_ijk);
            end
