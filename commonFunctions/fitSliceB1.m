function [] = fitSliceB1(nDessSlice,examPath,seriesNo,b1SliceLocations)
...
% Let's find the B1 slice that best matches this DESS slice location
dessSliceLoc = echo1H_info.SliceLocation;
[~,nB1Slice] = min(abs(dessSliceLoc-b1SliceLocations));
b1Map = alignedB1Map(examPath,seriesNo,nDessSlice,nB1Slice);



% Do the parameter fit.
[adcFitBieri_notCorr,t2Fit2D_notCorr,adcFit2D_notCorr] = DESS2dFit_imgB1(TR,TE,Tg,GhArea,GlArea,alpha_deg,Nstates,echo1H,echo2H,echo1L,echo2L,b1Map);

alpha_deg_L_map = echo1L_info.FlipAngle.*b1Map;
assumedT1 = 1.2;
t2FitWelschNew_notCorr = -2*(TR-TE)./log(echo2L./(echo1L .* (sind(alpha_deg_L_map/2)).^2 .* (1 + exp(-TR/assumedT1))./(1 - cosd(alpha_deg_L_map)*exp(-TR/assumedT1))));
...
str1 = ['adc_bieri_m_B1_sl',num2str(nDessSlice)];
str2 = 'adc_bieri_m_B1';
adcFitBieri_notCorr_info.FileName = ['outputDicoms/', str1, '.dcm'];
adcFitBieri_notCorr_info.SeriesNumber = adcFitBieri_notCorr_info.SeriesNumber*1000 + 220;
...
str1 = ['adc_2d_m_B1_sl',num2str(nDessSlice)];
str2 = 'adc_2d_m_B1';
adcFit2D_notCorr_info.FileName = ['outputDicoms/', str1, '.dcm'];
adcFit2D_notCorr_info.SeriesNumber = adcFit2D_notCorr_info.SeriesNumber*1000 + 230;
...
str1 = ['t2_2d_m_B1_sl',num2str(nDessSlice)];
str2 = 't2_2d_m_B1';
t2Fit2D_notCorr_info.FileName = ['outputDicoms/', str1, '.dcm'];
t2Fit2D_notCorr_info.SeriesNumber = t2Fit2D_notCorr_info.SeriesNumber*1000 + 250;
...
str1 = ['t2_Welsch_new_B1_sl',num2str(nDessSlice)];
str2 = 't2_Welsch_new_B1';
t2FitWelschNew_notCorr_info.FileName = ['outputDicoms/', str1, '.dcm'];
t2FitWelschNew_notCorr_info.SeriesNumber = t2FitWelschNew_notCorr_info.SeriesNumber*1000 + 270;
