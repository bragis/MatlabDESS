function [adcMap3D,t2Map3D,t1Map3D] = DESS3dFit_img(TRh,TEh,Tgh,TRl,TEl,Tgl,GhArea,GlArea,alphah_deg,alphal_deg,Nstates,e1_H,e2_H,e1_L,e2_L)

% Compute the gradient amplitudes in G/m
Gh = GhArea/(Tgh*1e6)*100;
Gl = GlArea/(Tgl*1e6)*100;
...
filestr = ['dataBases/signalDatabase3D', num2str(round(GhArea)), '.mat'];
...
if (exist(filestr) == 2)
    ...
    % its parameters.
    load(filestr);
    if ((dataBase3D.TRh == TRh) && (dataBase3D.TEh == TEh) && (dataBase3D.Tgh == Tgh) ...
        &&  (dataBase3D.TRl == TRl) && (dataBase3D.TEl == TEl) && (dataBase3D.Tgl == Tgl) ...
        && (dataBase3D.GhArea == GhArea) && (dataBase3D.GlArea == GlArea) ...
        && (dataBase3D.alphah_deg == alphah_deg) && (dataBase3D.alphal_deg == alphal_deg) ...
        && (dataBase3D.Nstates == Nstates) ...
        && isequal(dataBase3D.T1values,T1values) && isequal(dataBase3D.T2values,T2values) && isequal(dataBase3D.Dvalues,Dvalues))
...
%             r2Grid2D_alpha = dataBase2D.r2Grid2D_alpha;
            T1T2DDspace = dataBase3D.T1T2DDspace;
    else
        clear dataBase3D;
    end
end
if (matchingDatabaseFound == 0)
    ...
    dataBase3D.TEl = TEl;
    dataBase3D.Tgl = Tgl;
    dataBase3D.GhArea = GhArea;
    dataBase3D.GlArea = GlArea;
    dataBase3D.alphah_deg = alphah_deg;
    dataBase3D.alphal_deg = alphal_deg;
    dataBase3D.alphaFracValues = 1;
    dataBase3D.Nstates = Nstates;
    dataBase3D.T1values = T1values;
    dataBase3D.T2values = T2values;
    dataBase3D.Dvalues = Dvalues;
    disp('Creating grid...');
...
%         for (nt2 = 1:NT2)
%             for (nd = 1:ND)
%                 [SpH,SmH] = computeEchoesEPG(T1values(nt1),T2values(nt2),TRh,TEh,alphah_deg,Gh,Tgh,Dvalues(nd),Nstates);
%                 [SpL,SmL] = computeEchoesEPG(T1values(nt1),T2values(nt2),TRl,TEl,alphal_deg,Gl,Tgl,Dvalues(nd),Nstates);
% 
...
%     dataBase2D.r2Grid2D_alpha = r2Grid2D_alpha;
	...
	T1T2DDspace = generateSpace2(T1start,T1end,NT1,T2start,...
    T2end,NT2,Dstart,Dend,ND,TRh,TEh,Tgh,alphah_deg,Gh,...
    TRl,TEl,Tgl,alphal_deg,Gl,Nstates,2);
    
    dataBase3D.T1T2DDspace = T1T2DDspace;

    save(filestr,'dataBase3D');
end
...
[t2Map3D,adcMap3D,t1Map3D] = linearFitNbyM(e1_H,e2_H,e1_L,e2_L,T1T2DDspace);